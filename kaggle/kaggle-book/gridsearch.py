...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  8 15:52:52 2018
...
#parameters = {'svc_gamma':np.logspace(-2,1,4),'svc_C':np.logspace(-1,1,3)}#这一句是错的！！！！？？？？！！！
parameters = {'svc__gamma': np.logspace(-2, 1, 4), 'svc__C': np.logspace(-1, 1,3)}#svc__gamma两道下划线！！！

# 从sklearn.grid_search中导入网格搜索模块GridSearchCV。
from sklearn.grid_search import GridSearchCV


#默认为True,程序将会以交叉验证训练集得到的最佳参数，重新对所有可用的训练集与开发集进行，
...
gs = GridSearchCV(clf, parameters, verbose=2, refit=True, cv=3,n_jobs=-1)#并行计算
